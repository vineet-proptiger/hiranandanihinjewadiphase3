...
const slides = [
  '/heera/images/hero/banner1.webp',
  '/heera/images/hero/banner2.webp',
]
...
const Hero = ({ setIsOpen }) => {
  ...
  return (
    <section className="hero-container">
      ...
      {/* ── Image Banner Carousel (Desktop & Tablet) ── */}
      <div className="hero-slider-wrapper hidden md:block relative w-full h-full">
        {slides.map((src, idx) => (
          <img
            key={src}
            src={src}
            alt={`Hiranandani Hinjewadi Township Banner ${idx + 1}`}
            className={`hero-image desktop-hero-image w-full h-full object-cover block transition-opacity duration-1000 ${
              current === idx ? 'opacity-100 relative z-10' : 'opacity-0 absolute inset-0 z-0'
            }`}
          />
        ))}
      </div>

      {/* ── Image Banner Carousel (Mobile Devices) ── */}
      <div className="block md:hidden relative w-full bg-[#0F172A] aspect-video overflow-hidden">
        {slides.map((src, idx) => (
          <img
            key={src}
            src={src}
            alt={`Hiranandani Hinjewadi Township Mobile Banner ${idx + 1}`}
      ...
      <div className="hero-content">
        ...
        <p className="hero-subtitle">
          <span style={{ fontSize: '0.9em', fontWeight: 600, textTransform: 'none', color: '#fff' }}>Hinjewadi Phase 3 Krisala Everland, Pune</span>
        </p>
        ...
        <div className="hero-bullets" style={{ marginBottom: '24px', display: 'flex', flexDirection: 'column', gap: '8px' }}>
          {[
            ...
            'New Launch 2, 3 & 4 BHK Apartments',
            'Located on Pune IT & Growth Corridor',
            'At Hinjewadi Phase 3, Pune'
          ].map((text, i) => (
